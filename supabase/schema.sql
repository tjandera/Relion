-- Relion database schema
-- Run this once in the Supabase SQL editor for project qhdmubotldvbcihrlivs:
--   https://supabase.com/dashboard/project/qhdmubotldvbcihrlivs/sql/new
-- Paste the whole file and click Run. Safe to run on a fresh project.
--
-- It creates four tables (profiles, contacts, notes, messages), a trigger that
-- auto-creates a profile when a user signs up, and Row Level Security policies
-- that scope every row to the user who owns it.
...
-- Lock down the signup trigger function so it cannot be called directly by clients.
